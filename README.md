...
Протестировано с версией Python 3.8.5. Для запуска скрита используйте:

`$ python validate.py path-to-template path-to-doc-to-check`

Например:

`$ python validate.py templates/template1.jpg test_data/445.jpg`

Можно запускать для директории:

`$ python validate.py templates/template1.jpg test_data`

И даже использовать сразу несколько шаблонов:

`$ python validate.py templates test_data`
...
В коде, добавленном после дедлайна :-( используется решение, основанное на распознавании печатного текста, но пока что не оптимизированное. Чем более похожи документы, тем дольше происходит проверка. В дальнешем планируется использовать более эффективные по скорости методы распознавания текста.
...
## Распределение задач
Александр - получение данных из Толоки, проверка соответствия документа, инструмент для разметки шаблонов
